...
const firebaseConfig = window.MAHJONG_FIREBASE_CONFIG;
const MAHJONG_URL = "https://potoro1001.github.io/potoro-mahjong/mahjong-room-list.html";
  ...
  if(listEl) {
    listEl.innerHTML = openRooms
      ...
      .map((room) => {
        ...
        return `<span class="tag room-tag">${escapeHtml(label)} / 残り${remain}名</span>`;
      })
      .join("");
  }
...
async function startMahjongFeed(auth, db) {
  try {
    await signInAnonymously(auth);
  } catch (err) {
    console.error("Mahjong auth failed:", err);
    hideMahjongUi();
    return;
  }
  ...
  onSnapshot(
    ...
    (err) => {
      console.error("Mahjong snapshot failed:", err);
      hideMahjongUi();
    }
  );
}
...
if(!firebaseConfig?.apiKey || !firebaseConfig?.projectId) {
  ...
  startMahjongFeed(getAuth(app), getFirestore(app));
}
